        ...
        if shape:  # reward can optionally be shaped (decreased reward for increased distance from agents)
            ...
            for adv in adversaries:   #对应的是每一个通信无人机
                ...
                for a in adv_dict[adv]:
                    sum_connect += 1  #这里是固定带宽  每个用户所需阈值  这里后续自定义用户的需求数组
                # print ("adv == ",adv)
                # print ("sum == ",sum)
                dist_punish = sum([np.sqrt(np.sum(np.square(a.state.p_pos - adv.state.p_pos))) for a in adv_dict[adv]])   #这里的距离惩罚有问题  应该处理的是连接的节点的距离
                if sum_connect > 2:
                    # rew -= 2
                    success = False
                    rew -= 15 * sum_connect  #这里是为了平衡dist
                else:
                    rew += 10 * sum_connect  #下次改的话   就是将这里的dist_publish 规约成常量  不能用这个变量操作的"""  """
                    # rew += sum_connect
                # rew -= 0.1 * sum([np.sqrt(np.sum(np.square(a.state.p_pos - adv.state.p_pos))) for a in agents])
                rew -= dist_punish  #已经算进距离了  这里不用了
            if success == True:
                rew += 50 #感觉还行  感觉500更好？
#缺了一个完成任务的reward  这是maddpg的关键

                #find出最近的无人机节点  如果对应节点的阈值超出了无人机的范围  那么给予负奖励  否则给正奖励  但是想一想  这会让三个min继续min嘛  试试吧
                #这就相当于在多维调控了
        if agent.collide:
            for ag in agents:
                for adv in adversaries:
