...
deploy_orgin_pos = [[53.0, 234.0], [182.0, 192.0], [138.0, 224.0], [136.0, 271.0], [211.0, 308.0], [202.0, 355.0], [215.0, 438.0], [180.0, 203.0], [90.0, 391.0], [78.0, 287.0], [265.0, 324.0], [343.0, 415.0], [207.0, 263.0], [178.0, 55.0], [330.0, 366.0], [73.0, 360.0], [146.0, 144.0], [183.0, 353.0], [384.0, 435.0], [433.0, 267.0]]
...
class Scenario(BaseScenario): #在reset的时候 修改用户比例
    ...
    def reset_world(self, world):  #reset world的时间  夹在两次查询中间  就会出现bug了
        #并且这里返回的是上一轮的位置的部署结果  所以不可行
        # writeFile = False  #这个状态会根据时间有一定的延迟  如何解决呢
        if self.line_cnt == 2 and os.path.getsize(web_input_path):
            with open(web_input_path) as f:
                self.location_dict = json.load(f)   #load针对文件  loads针对内存对象
                # writeFile = True
                self.line_cnt = 1
            open(web_input_path,'w').close()  #读取后清除
        else:
            #读取本地文件
            with open('/home/zk/maddpg/com_navi_latest_for_ubuntu/maddpg_communicate_navigation_uav_latest/multiagent-particle-envs-master/multiagent/scenarios/location20_single.json') as f:
                self.location_dict = json.load(f)
            # open(web_output_path,'w').close()  #读取后清除
        ...
        print("line_cnt:",self.line_cnt)
        if self.line_cnt == 1 or self.line_cnt == 0:
            if self.line_cnt == 1:
                self.line_cnt = 0
            elif self.line_cnt == 0:
                self.line_cnt = 2
            with open(web_output_path, 'a+',newline='\n') as f:
                if len(new_dict['uav_pos']) != 0:
                    f.write(data)                    
                    f.write('\n')
        for i, agent in enumerate(world.agents):
        ...
        for i, landmark in enumerate(world.landmarks):
            if not landmark.boundary:
                pos_x = (self.location_dict['user_location'][i][0] - 250) / 250
                pos_y = (self.location_dict['user_location'][i][1] - 250) / 250
                pos = [pos_x,pos_y]
