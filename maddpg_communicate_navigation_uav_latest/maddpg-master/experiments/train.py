...
def parse_args():
    ...
    parser.add_argument("--num-episodes", type=int, default=80000, help="number of episodes") #60000 - 30000
    ...
    parser.add_argument("--save-dir", type=str, default="./policy/", help="directory in which training state and model should be saved") #/tmp/policy
            ...
            if terminal and (len(episode_rewards) % arglist.save_rate == 0):
                ...
                if num_adversaries == 0:
                    print("steps: {}, episodes: {}, mean episode reward: {}, com_navi_reward: {},time: {}, success:{}".format(
                        train_step, len(episode_rewards), np.mean(episode_rewards[-arglist.save_rate:]), np.sum(episode_com_nai[-arglist.save_rate:])/1000 ,round(time.time()-t_start, 3),np.sum(episode_success_sum[-arglist.save_rate:])))
                else:
