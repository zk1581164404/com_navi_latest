...
class SoftCategoricalPdType(PdType): #离散动作数
    def __init__(self, ncat):
        self.ncat = ncat
    def pdclass(self):
...
def make_pdtype(ac_space):
    ...
    #space.box在n维空间中，一个box代表n维封闭区间的笛卡尔积  每个空间都有[a,b],(-oo,n),[a,+oo),或者(-oo,+oo)的形式
    if isinstance(ac_space, spaces.Box):
        assert len(ac_space.shape) == 1
        return DiagGaussianPdType(ac_space.shape[0])
    elif isinstance(ac_space, spaces.Discrete):  #单智能体离散动作空间
        # return CategoricalPdType(ac_space.n)
        return SoftCategoricalPdType(ac_space.n)
    elif isinstance(ac_space, MultiDiscrete):  #多智能体离散动作空间
