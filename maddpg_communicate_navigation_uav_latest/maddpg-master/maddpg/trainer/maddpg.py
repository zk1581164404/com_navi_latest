...
def discount_with_dones(rewards, dones, gamma):
    discounted = []
    r = 0
    for reward, done in zip(rewards[::-1], dones[::-1]):   #进行逆序操作
        r = reward + gamma*r
        r = r*(1.-done)
        discounted.append(r)
    return discounted[::-1]  #逆序后是时间从先到后的顺序

#更新target网络参数（软更新）
def make_update_exp(vals, target_vals):
    polyak = 1.0 - 1e-2
    expression = []
    #lambda匿名函数  对元素列表进行排序
    for var, var_target in zip(sorted(vals, key=lambda v: v.name), sorted(target_vals, key=lambda v: v.name)):
        expression.append(var_target.assign(polyak * var_target + (1.0-polyak) * var))  #assign给var_target添加一列
    expression = tf.group(*expression) # * 的作用是把列表expression中的每个元素 当做位置参数传入 如group（1,2,3）
    return U.function([], [], updates=[expression])


#建立actor，建立critic 然后将critic的输出和actor输出当做loss来训练actor
#建立policy（actor）网络结构和训练过程
def p_train(make_obs_ph_n, act_space_n, p_index, p_func, q_func, optimizer, grad_norm_clipping=None, local_q_func=False, num_units=64, scope="trainer", reuse=None):
    with tf.variable_scope(scope, reuse=reuse):  #用于定义创建变量的操作的上下文管理器
        # create distribtuions
        act_pdtype_n = [make_pdtype(act_space) for act_space in act_space_n] #设定action的概率分布类型和概率的值

        # set up placeholders
        obs_ph_n = make_obs_ph_n  #p网络的输入为当前无人机的局部状态
        act_ph_n = [act_pdtype_n[i].sample_placeholder([None], name="action"+str(i)) for i in range(len(act_space_n))]

        p_input = obs_ph_n[p_index]

        p = p_func(p_input, int(act_pdtype_n[p_index].param_shape()[0]), scope="p_func", num_units=num_units)  #输入无人机的当前状态  输出策略分布
        ...
        act_input_n = act_ph_n + []   #critic网络的输入
        act_input_n[p_index] = act_pd.sample()  #将p网络的输出动作给相应智能体作为q网络的输入
        q_input = tf.concat(obs_ph_n + act_input_n, 1)
        # critic网络的输入所有agent的观测和动作，在1维度concat obs_ph_n和act_input_n=（x',a1,a2,a3...aN）
        # 如果是DDPG，就将local_q_func标记为True，并将该智能体的局部observation和actions传入给critic网络

        #这里就是maddpg与ddpg的区别  是否接受其他智能体的动作
        ...
        # 返回输出的动作,训练loss,网络参数，策略奖励和target网络的输出动作  这里loss可以画出来吧
...
def q_train(make_obs_ph_n, act_space_n, q_index, q_func, optimizer, grad_norm_clipping=None, local_q_func=False, scope="trainer", reuse=None, num_units=64):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        q_input = tf.concat(obs_ph_n + act_ph_n, 1) # q网络的输入为全局观测和全局动作
        if local_q_func:
